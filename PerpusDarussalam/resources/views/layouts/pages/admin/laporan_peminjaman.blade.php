            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <div class="inline-flex bg-[#004d40] p-1 rounded-lg border border-[#004d40] shadow-sm">
                        <a href="{{ route('laporan.peminjaman', ['date' => $selectedDate->format('Y-m-d'), 'mode' => 'harian']) }}" 
                        ...
                        <a href="{{ route('laporan.peminjaman', ['date' => $selectedDate->format('Y-m-d'), 'mode' => 'mingguan']) }}" 
                    ...
                    @if($mode === 'harian')
                        <div class="inline-flex bg-[#004d40] rounded border border-[#004d40] overflow-hidden shadow-sm">
                            @foreach($dates as $d)
                                <a href="{{ route('laporan.peminjaman', ['date' => $d['full_date'], 'mode' => 'harian']) }}" 
                    ...
                    <form action="{{ route('laporan.peminjaman') }}" method="GET" class="flex items-center relative">
                        <input type="hidden" name="mode" value="{{ $mode }}">
                        <button type="button" 
                                onclick="document.getElementById('date-picker-peminjaman').showPicker()" 
                        ...
                        <input type="date" id="date-picker-peminjaman" name="date" class="opacity-0 absolute pointer-events-none w-0 h-0" onchange="this.form.submit()" value="{{ $selectedDate->format('Y-m-d') }}">
                    </form>
                </div>

                <!-- Kanan: Tombol Kembali & Print -->
