...
<div class="max-w-6xl mx-auto px-4 py-8 space-y-8">
    ...
    <!-- Tombol Kembali -->
    <div>
        <a href="{{ route('user.home') }}" class="inline-flex items-center gap-2 bg-[#002820] text-emerald-200 hover:text-white px-4 py-2 rounded-xl border border-emerald-700/60 text-xs font-semibold shadow transition hover:bg-emerald-900">
    ...
    <div class="bg-[#00382e] text-white rounded-2xl p-6 md:p-10 shadow-2xl border border-emerald-700/60 grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
        
        <!-- Kolom Cover Buku -->
        <div class="w-full">
            <div class="w-full h-80 md:h-[400px] bg-[#001f19] rounded-xl overflow-hidden shadow-inner border border-emerald-800/60 relative group">
                ...
                @if(data_get($book, 'categories.nama'))
                    <span class="absolute top-3 right-3 bg-[#004d40] text-emerald-100 text-xs font-bold px-3 py-1 rounded-full shadow border border-emerald-600/50">
            ...
            <!-- Status Stok -->
            <div class="mt-4 text-center bg-[#002820] p-3 rounded-xl border border-emerald-700/60">
        ...
        <div class="md:col-span-2 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 py-4 border-y border-emerald-800/80 text-xs">
                <div>
                    <span class="text-emerald-300/70 block">Penerbit</span>
                    <span class="font-bold text-white text-sm">{{ data_get($book, 'penerbit', '-') }}</span>
                </div>
                <div>
                    <span class="text-emerald-300/70 block">Tahun Terbit</span>
                    <span class="font-bold text-white text-sm">{{ data_get($book, 'tahun_terbit', '-') }}</span>
                </div>
                <div>
                    <span class="text-emerald-300/70 block">ISBN</span>
                    <span class="font-bold text-white text-sm">{{ data_get($book, 'isbn', '-') }}</span>
                </div>
                <div>
                    <span class="text-emerald-300/70 block">Lokasi Rak</span>
                    <span class="font-bold text-white text-sm">{{ data_get($book, 'rak', '-') }}</span>
                </div>
                <div>
                    <span class="text-emerald-300/70 block">Tanggal Pembelian</span>
